...
export async function POST(req: Request) {
  try {
    ...
    await SendAdminNotification(
      `${account.name} đã đăng ký tư vấn trường học ${existingSchool.name}`,
      `${account.name} vừa đăng ký tài khoản mới (${account.id})`,
      `${account.name}`,
      `/accounts/${account.id}`
    );
    ...
    return NextResponse.json(
      {
        account,
        message:
          "Đăng ký thành công, vui lòng kiểm tra hòm thư email để xác thực người dùng",
      },
      { status: 200 }
    );
  } catch (error) {
