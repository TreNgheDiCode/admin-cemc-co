...
import { SchoolGalleryLib } from "@/types/school";
...
type Props = {
  galleries: SchoolGalleryLib[];
  schoolId: string;
};

export const SchoolGalleriesList = ({ galleries, schoolId }: Props) => {
  const [active, setActive] = useState<
    (typeof galleries)[number] | boolean | null
  >(null);
  const id = useId();
    ...
    <>
              ...
              <div>
                <div className="flex justify-between items-start p-4">
                  ...
                  <motion.a
                    ...
                    animate={{ opacity: 1 }}
                    exit={{ opacity: 0 }}
                    href={`/schools/${schoolId}/gallery/${active.id}`}
                    target="_blank"
      ...
      <ul className="mx-auto w-full grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 items-start gap-4">
        ...
        <div className="size-full flex items-center justify-center">
          <button
            onClick={() => router.push(`/schools/${schoolId}/galleries/`)}
