...
import { SchoolFeedbackReply } from "@/types/school";
...
type Props = {
  feedbacks: SchoolFeedbackReply[];
};

export const SchoolFeedbacksWrapper = ({ feedbacks }: Props) => {
  const { user } = useUser();
  ...
  const onVisitFeedback = useCallback(
    async (feedback: SchoolFeedbackReply) => {
      const params = new URLSearchParams(searchParams.toString());
