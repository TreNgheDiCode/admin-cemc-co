...
type InputProps = {
  ...
  onChange: (file?: SingleFileDropzone[]) => void | Promise<void>;
  onFilesAdded?: (addedFiles: SingleFileDropzone[]) => void | Promise<void>;
  disabled?: boolean;
...
const ERROR_MESSAGES = {
  fileTooLarge(maxSize: number) {
    return `File vượt quá dung lượng cho phép: ${formatFileSize(maxSize)}.`;
  },
  fileInvalidType() {
    return "Loại tập tin không hợp lệ.";
  },
  tooManyFiles(maxFiles: number) {
    return `Bạn chỉ có thể thêm tối đa ${maxFiles} file.`;
  },
  fileNotSupported() {
    return "Loại tập tin không được hỗ trợ.";
  },
};
  ...
  (
    ...
    const {
      ...
      onDrop: (acceptedFiles) => {
        ...
        if (files) {
          const addedFiles = files.map<SingleFileDropzone>((file) => ({
            file,
